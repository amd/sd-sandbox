...
def setup_execution_provider(args: argparse.Namespace, config: Dict[str, Any]):
    ...
    if force_cpu:
        ...
        print("🖥️  CPU-only execution enabled:")
        print("   • GPU providers disabled (DML)")
        print("   • All accelerators will fall back to CPU")
    else:
        print("[AUTO] Automatic provider selection enabled")
        print("   • Will use best available: DML > CPU")
    ...
    try:
        ...
        print("   • ONNX Runtime not available for provider detection")
...
def setup_pipeline_execution_provider(config_item: Dict[str, Any], args: argparse.Namespace, config: Dict[str, Any]):
    ...
    if force_cpu:
        ...
        execution_info = f"🖥️  {config_item['name']} - CPU-only execution"
...
def handle_sd3_controlnet_mode(script_name: str, prompt_file_path: Optional[str],
    ...
    """
    if script_name not in ("run_sd3.py", "run_sd3_controlnet_outpainting.py") or not prompt_file_path or "--controlnet" not in extra_args:
