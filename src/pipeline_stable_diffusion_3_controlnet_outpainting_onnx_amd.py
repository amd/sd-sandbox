...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers.pipelines import StableDiffusion3ControlNetOutpaintingPipeline
        >>> from diffusers.models.controlnet_sd3 import SD3ControlNetModel

        >>> controlnet = SD3ControlNetModel.from_pretrained(
        ...     "alimama-creative/SD3-Controlnet-Inpainting", use_safetensors=True, extra_conditioning_channels=1
        ... )
        >>> pipe = StableDiffusion3ControlNetOutpaintingPipeline.from_pretrained(
...
class StableDiffusion3ControlNetOutpaintingPipeline(
    DiffusionPipeline,
    SD3LoraLoaderMixin,
    FromSingleFileMixin,
):
    ...
    def _get_t5_prompt_embeds(
        ...
        logger.debug(f"tokenizer_3 inference time = {time.perf_counter() - t0:.5f}s")
    ...
    def _get_clip_prompt_embeds(
        ...
        logger.debug(f"{tokenizer_name} inference time = {t1 - t0:.5f}s")
    ...
    def prepare_image_with_mask(
        ...
        image_latents = self.vae_encoder(sample=masked_image)[0]
        logger.debug(f"vae encoder time {time.perf_counter() - vae_en_start_time}")
        self.perf_time_dict["vae_encoder"].append(
            time.perf_counter() - vae_en_start_time
        )
        image_latents = DiagonalGaussianDistribution(
            torch.from_numpy(image_latents).to(device)
        ).sample()
        image_latents = (
            image_latents - self.vae_encoder.config["shift_factor"]
        ) * self.vae_encoder.config["scaling_factor"]
        ...
        return control_image, origin_image
    ...
    def __call__(
        ...
        logger.debug(f"max_sequence_length : {max_sequence_length}")
        ...
        if isinstance(self.controlnet, OnnxRuntimeModel):
            ...
            # control_image = DiagonalGaussianDistribution(control_image).sample()
        ...
        if control_name == "inpainting":
            controlnet_prompt_embeds = prompt_embeds.cpu().numpy()
        else:
            controlnet_prompt_embeds = torch.zeros_like(prompt_embeds).cpu().numpy()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if XLA_AVAILABLE:
                    xm.mark_step()
        logger.debug(
            f"controlnet + transformer inference total time ({num_inference_steps} steps) = {time.perf_counter() - t0:.3f}s"
        )
        vae_decoder_start_time = time.perf_counter()
        latents = (
        ...
        logger.debug(
            f"vae_decoder inference time = {time.perf_counter() - vae_decoder_start_time:.3f}s"
        )
        self.perf_time_dict["vae_decoder"].append(
            time.perf_counter() - vae_decoder_start_time
        )
