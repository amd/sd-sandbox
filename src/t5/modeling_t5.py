...
if "mha_npu" in os.environ and os.environ["mha_npu"] == "1":
    # define npu mha
    t5_mha_op = atom.atom_t5_npu_mha()
    padded_query_states = torch.zeros((64, 128, 64), dtype=torch.bfloat16)
    padded_key_states = torch.zeros((64, 128, 64), dtype=torch.bfloat16)
    padded_value_states = torch.zeros((64, 128, 64), dtype=torch.bfloat16)
    padded_am_states = torch.zeros((64, 128, 128), dtype=torch.bfloat16)
    padded_am_states[:, :, :] = -float("inf")
bmm1_op = atom.atom_npu_bmm(True)
bmm2_op = atom.atom_npu_bmm(False)
padded_query_states = torch.zeros((64, 128, 64), dtype=torch.bfloat16)
padded_key_states = torch.zeros((64, 128, 64), dtype=torch.bfloat16)
padded_value_states = torch.zeros((64, 128, 64), dtype=torch.bfloat16)
padded_attn_weights = torch.zeros((64, 128, 128), dtype=torch.bfloat16)
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        M = query_states.shape[-2]
        padded_query_states[:, :M, :] = query_states.to(torch.bfloat16)
        padded_key_states[:, :M, :] = key_states.to(torch.bfloat16)
        # print('qkv: {}'.format(time.perf_counter()-start_time))
        scores = bmm1_op.execute(padded_query_states, padded_key_states).unsqueeze(0)
        scores = scores[:, :, :M, :M].to(torch.float16)
        ...
        if "mha_npu" in os.environ and os.environ["mha_npu"] == "1":
            # start ryzenai mha
            M = query_states.shape[-2]
            padded_query_states[:, :M, :] = query_states
            padded_key_states[:, :M, :] = key_states
            padded_value_states[:, :M, :] = value_states
            padded_am_states[:, :M, :M] = position_bias_masked
            attn_output = t5_mha_op.execute(
                padded_query_states,
                padded_key_states,
                padded_value_states,
                padded_am_states,
            ...
            # d = torch.matmul(attn_weights, value_states)
            # attn_output = unshape(d)  # (batch_size, seq_length, dim)

            padded_attn_weights[:, :M, :M] = attn_weights.to(torch.bfloat16)
            padded_value_states[:, :M, :] = torch.squeeze(value_states, dim=0).to(
                torch.bfloat16
            )
            aie_attn_output = bmm2_op.execute2(padded_attn_weights, padded_value_states)
            aie_attn_output = aie_attn_output[:M, :, :]
            attn_output = aie_attn_output.view(batch_size, -1, self.inner_dim)

            # a = torch.sum(aie_attn_output) - torch.sum(attn_output)
            # aie_attn_output = unshape(aie_attn_output[:, :M, :]).to(torch.float16)
